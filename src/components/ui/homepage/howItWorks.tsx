...
import { AnimatePresence, motion, useMotionTemplate, useMotionValueEvent, useScroll, useSpring, useTransform } from "framer-motion";
...
export default function HowItWorks() {
  ...
  return (
    <section  ref={containerRef} className="relative w-full min-h-[300vh] pt-[22vh]">
      <img
      ...
      <div className="relative">
        <div className="max-w-345 mx-auto text-center flex flex-col justify-center items-center px-5 md:px-8">
          <SectionHeader 
            tag="how it works"
            headline="Turn any idea into ready-to-use content in seconds"
      ...
      <motion.div
       className="sticky top-24 md:top-40 overflow-hidden p-4 md:p-6">
        <div className="mx-auto flex w-full max-w-340 flex-col gap-4 lg:flex-row lg:items-start lg:gap-0">

          {/* Cards Left column */}
          <div className="flex w-full flex-col gap-4 lg:w-[489.59px] lg:shrink-0 lg:gap-0 ">
            {HOW_IT_WORKS_STEPS.map((s, i) => {
              const active = i == activeIndex;
              return (
                <div key={s.step} className="w-full shrink-0 lg:p-2.5 border border-(--border-color) not-last:border-b-0 border-r-0 ">
                  <div
                    className={
                      "flex min-h-40 flex-col gap-6 rounded-xl border border-(--border-color) p-5 sm:p-6 lg:h-47.25 lg:gap-7.5  " +
                      (active
                        ? "bg-white shadow-lg"
                        : "")
                    }
                  >
                    ...
                    <div className="flex flex-col gap-3">
                      <h3 className="text-xl sm:text-2xl font-medium tracking-tight text-(--secondary-color)">
                        {s.heading}
                      </h3>
          ...
          {/* Right column */}
          <div className="flex w-full flex-col border border-(--border-color) lg:w-218 lg:shrink-0">
            <div className="flex h-10 shrink-0 items-center justify-center border-b border-(--border-color) px-2 lg:h-[38.41px]">
              <p className="text-center text-xs italic text-[#333637]">
                {activeStep.tagline}
              </p>
            </div>

            <div className="flex h-65 shrink-0 sm:h-90 lg:h-[590.98px]">
              {/* left narrow column */}
              <div className="hidden shrink-0 flex-col  lg:flex lg:w-[130.55px]">
              ...
              <div className="flex w-full shrink-0 flex-col border-(--border-color) lg:w-[609.31px] lg:border-r">
                <div className="hidden h-29.5 shrink-0 p-2.5 lg:block">
                  <div className="h-full rounded-xl border border-(--border-color)" />
                </div>
                  ...
                  <div className="h-full w-full overflow-hidden rounded-xl border border-(--border-color)">
                   <AnimatePresence mode="wait">
                      <motion.img
                        key={activeIndex}
                        src={
                          `/howItWorks_${activeIndex + 1}.avif`
                        }
